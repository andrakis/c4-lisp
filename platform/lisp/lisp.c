...
enum /* CellType */ {
	...
	Reference=0x40,   // V_VAL => cell*
	OnStack = 0x80    // 1 = was allocated on the stack, 0 = malloc
};
// Useful enums (avoids magic numbers elsewhere)
enum { CellTypeMask = 0x5F, NoAlt = 0 };
...
void cell_free (int *cell) {
	cell_deinit(cell);
	free(cell); /* lol */
}
void v_cell_free1 (int bp) {
	int *cell;

	cell = bp - V__SZ;
	cell_deinit(cell);
	// free references (recursively) that are on stack
	if(cell_is_reference(cell) && cell[V_VAL])
		if(cell_is_onstack((int*)cell[V_VAL])) v_cell_free1(cell[V_VAL]);
		else cell_free((int*)cell[V_VAL]);
	}
	v_free(V__SZ);
}
void v_cell_free () { v_cell_free1(v_sp); }
...
int cell_is_reference(int *cell) { return cell[V_TYPE] & Reference; }
...
void environment_deinit (int *env) {
	...
	if(debug)
		dprintf(Stderr, "    ~environment at 0x%X\n", env);
	if(env[E_MEMBERS]) {
		members = (int*)env[E_MEMBERS];
		if(!cell_is_onstack(members))
			cell_free(members);
	}
}
...
int *v_env () {
	...
	// allocate MEMBERS on the stack
	env[E_MEMBERS] = v_cell_new(List | Reference, v_cell_new(List | String, "end$", NoAlt), NoAlt);
	return env;
}
void v_env_free () {
	int *e;
...
void v_env_push (int *source) {
	...
	dest = v_env();
	dest[E_OUTER] = source[E_OUTER];
	dest[E_MEMBERS] = source[E_MEMBERS];
}
